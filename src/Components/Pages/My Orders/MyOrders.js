...
const MyOrders = () => {
    ...
    useEffect(()=>{
        const url =`https://hidden-falls-85514.herokuapp.com/allorders?email=${user.email}`
        fetch(url)
        .then(res => res.json())
                ...
                {orderedUser.map((person) => (
                  <tr key={(person.orderedId) || (person.price)}>
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <div className="flex items-center text-center">
                        <div className="flex-shrink-0 h-10 w-10">
                            {person.img && <img className="h-10 w-10 rounded-full" src={person.img} alt="" />
}
                            {
                                !person.img && <i className="fas fa-user-circle text-white text-3xl"></i>
                            }
                        </div>
                        <div className="ml-4">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap ">
                      ...
                      <div className="text-sm text-gray-900 font-semibold">{person.serviceName.slice(0, 19)}</div>
                      <div className="text-sm text-gray-500">{person.orderedId}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap ">
                    
                      <div className="text-sm text-gray-900 font-semibold">{person.phone}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
